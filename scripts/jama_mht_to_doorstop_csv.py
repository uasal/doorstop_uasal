...
Date extraction sources (in priority order, highest first):
  - Priority 1 (highest): Table row dates — per-requirement dates that appear
    ...
    "Created" and a value cell containing the date string).
  - Priority 2 (fallback): Standalone text dates — date lines that appear
    outside the requirement tables in the HTML body, e.g.::

        Created: 02/09/2025 09:23:45 PM UTC
        Updated: 03/23/2026 06:05:49 PM UTC

    These are the actual JAMA per-requirement dates in many MHT exports.
    Applied only when a requirement table has no date rows of its own.

  Office XML document properties (``<o:Created>``, ``<o:LastSaved>``) in the
  MHT ``<head>`` are intentionally ignored — those are export-file timestamps
  generated by JAMA/Word when the file was saved, not requirement dates.
...
def parse_tables(html_content: str) -> List[Dict[str, str]]:
    ...
    Date fields are filled from two sources in priority order (highest first):
    ...
    for table_idx, rows in enumerate(parser.tables):
        ...
        if record and is_requirement:
            ...
            for canonical_date_col in ("created", "modified"):
                ...
                if not has_date and canonical_date_col in standalone_dates:
                    record[canonical_date_col] = standalone_dates[canonical_date_col]
                    log.debug(
                        "Table %d: injected standalone fallback %s = %r",
                        table_idx,
                        canonical_date_col,
